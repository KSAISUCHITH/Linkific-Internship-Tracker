...
export default function BookDetailsPage() {
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      if (willBeFavorite) {
        const { data } = await api.post(`/favorites/${book.id}`)
        setBook((prev) => ({ ...prev, is_favorite: data.is_favorite }))
      } else {
        const { data } = await api.delete(`/favorites/${book.id}`)
        setBook((prev) => ({ ...prev, is_favorite: data.is_favorite }))
      }
    } catch (err) {
  ...
  const handleRate = async (newRating) => {
    ...
    try {
      const { data } = await api.post(`/books/${book.id}/rating`, {
        rating: newRating
      })
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert(getApiErrorMessage(err, 'Unable to delete book.'))
    }
  }
  ...
  const handleSaveEdit = async (bookData) => {
    try {
      ...
      setError(getApiErrorMessage(err, 'Unable to update book.'))
      alert(getApiErrorMessage(err, 'Unable to update book.'))
    }
  }
